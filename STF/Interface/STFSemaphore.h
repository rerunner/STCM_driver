...
class STFSignalable
   {
   public:
   virtual ~STFSignalable() {}; // NHV: Added for g++ 4.1.1
   //! Set the signal
...
class STFWaitable
   {
   public:
   virtual ~STFWaitable() {}; // NHV: Added for G++ 4.1.1
...
class STFTimeoutWaitable : public STFWaitable
   {
   public:
   virtual ~STFTimeoutWaitable() {}; // NHV: Added for g++ 4.1.1
...
/// @class STFTimeoutSemaphore
/// @brief A standard semaphore which is upon creation initialized to 0
class STFTimeoutSemaphore : public STFSignalable, public STFTimeoutWaitable
   {
   protected:
   OSSTFTimeoutSemaphore osSemaphore;

   public:
   STFTimeoutSemaphore (void);
   virtual ~STFTimeoutSemaphore (void);
   ...
   /// @brief Wait for the semaphore
   /// The duration parameter is the relative time from the point in time where this function
   /// is called to the timeout.
   virtual STFResult WaitTimeout(const STFLoPrec32BitDuration & duration);
   };
...
inline STFTimeoutSemaphore::STFTimeoutSemaphore (void) : osSemaphore() { }

inline STFTimeoutSemaphore::~STFTimeoutSemaphore (void) { }

inline STFResult STFTimeoutSemaphore::Reset(void) { STFRES_RAISE(osSemaphore.Reset()); }

inline STFResult STFTimeoutSemaphore::Signal(void) { STFRES_RAISE(osSemaphore.Signal()); }

inline STFResult STFTimeoutSemaphore::Set(void) { STFRES_RAISE(osSemaphore.Signal()); }

inline STFResult STFTimeoutSemaphore::Wait(void) { STFRES_RAISE(osSemaphore.Wait()); }

inline STFResult STFTimeoutSemaphore::WaitImmediate(void) { STFRES_RAISE(osSemaphore.WaitImmediate()); }

inline STFResult STFTimeoutSemaphore::WaitTimeout(const STFLoPrec32BitDuration & duration) { STFRES_RAISE(osSemaphore.WaitTimeout(duration)); }
