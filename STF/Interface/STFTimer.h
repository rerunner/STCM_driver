...
static const uint32 STFTIMER_MAX_EVENTS = 64;   // must be a power of two!
...
class STFTimer	: public STFThread
	...
	private:
		...
		STFMutex				mutex;
		uint32 count;		// used to create a unique timer ID
	...
	public:
		...
		struct PendingEvents
			...
			} pendingEvents[STFTIMER_MAX_EVENTS];
		...
		*/
		...
		STFResult ScheduleRecurrentEvent(const STFHiPrec64BitTime & time, const STFHiPrec64BitDuration & dueCycleDuration, STFMessageSink * sink, const STFMessage & message, uint32 & timer);
		...
		*/
		STFResult CancelEvent(STFMessageSink * sink);
