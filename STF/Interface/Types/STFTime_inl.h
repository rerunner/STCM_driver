...
inline STFHiPrec64BitDuration STFHiPrec64BitDuration::GetAbsoluteDuration(void) const
   {
   return STFHiPrec64BitDuration((this->duration).Abs(), STFTU_108MHZTICKS);
   }
...
inline STFDate::operator STFString() const
   {
	STFString seperator("-");
   STFString s = STFString(year) + seperator + STFString(month, 2) + seperator + STFString(day);   
   return s;
   }
...
inline STFTimeOfDay::operator STFString() const
   {
	STFString seperator(":");
   STFString s = STFString(hour,2) + seperator + STFString(minute, 2) + seperator + STFString(second, 2);   
   return s;
   }
...
inline int32 ConvertClock108ToSystem(const int32 & duration)
	...
	if (uduration & 0x80000000)
		...
		sign = true;
		if (uduration == 0x80000000)
			uduration = 0x7fffffff;
		else
			uduration = (int32)uduration * (-1);
		}

	MUL32x32(uduration, ClockMultiplier_108ToSystem, dhigher, dlower);

#if _DEBUG
	uint32 max = (1 << (32-14))-1;
	if (dhigher > max)
		{
		DP("ConvertClock108ToSystem int32 DynamicRange exceeded\n");
		}
#endif
...
inline int32 ConvertClockSystemTo108(const int32 & duration)
	...
	MUL32x32(uduration, ClockMultiplier_SystemTo108, dhigher, dlower);
	
#if _DEBUG
	uint32 max = (1 << (32-18))-1;
	if (dhigher > max)
		{
		DP("ConvertClockSystemTo108 int32 DynamicRange exceeded\n");
		}
#endif
