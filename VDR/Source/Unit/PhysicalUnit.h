...
class PhysicalUnit : public VDRBase,
	...
	protected:
		...
		uint32 GetNumberOfParameters(uint64 * createParams);

		/// Get the nth creation parameter
		STFResult GetDWordParameter(uint64 * createParams, uint32 index, uint32 & param);
		STFResult GetStringParameter(uint64 * createParams, uint32 index, char * & param);
		STFResult GetPointerParameter(uint64 * createParams, uint32 index, void * & param);
...
class SharedPhysicalUnit : public PhysicalUnit
	...
	public:
		...
		virtual int GetNumCurrentUnits (void) {return numCurrentUnits;}
		//
		// Part of IPhysicalUnit implementation
		//
		virtual STFResult ActivateAndLock(IVirtualUnit * vunit, uint32 flags, const STFHiPrec64BitTime & time, const STFHiPrec32BitDuration & duration, const STFHiPrec64BitTime & systemTime);
...
class SharedPhysicalAutoMutex
	...
	public:
		SharedPhysicalAutoMutex(SharedPhysicalUnit* pU, STFMutex* mutex);
		~SharedPhysicalAutoMutex(void);
	};
