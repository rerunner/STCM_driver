...
#ifdef _DEBUG_CONFIGURE_TAGS // intentional ifdef
#if ! _DEBUG_CONFIGURE_TAGS
#undef _DEBUG_CONFIGURE_TAGS
#define _DEBUG_CONFIGURE_TAGS 0 && _DEBUG // allow manual override, see ITagUnit.h
#endif
#endif
...
#if _DEBUG_CONFIGURE_TAGS
#define DP_CONFIGURE(string) \
	do { \
		if ( (this->GetUnitID() >= _DEBUG_LOG_FIRST_UNIT_ID) && (this->GetUnitID()	<= _DEBUG_LOG_LAST_UNIT_ID) ) \
			DP("%s : "string, DebugUnitNameFromID(this->GetUnitID())); \
		} while (0)
#else
#define DP_CONFIGURE(string) do {} while (0)
#endif

#if _DEBUG_CHANGESET_WATCHPOINT && _DEBUG
// see ITagUnit.h
uint32	* pDebugChangedWatchPoint = NULL;
uint32	lastChangedWatchPointValue = 0;
void DebugWatchPointFunction(char *str, uint32 lastValue, uint32 * pCurrentValue, char *filename)
	{
	DP("WATCHPOINT(%s) change from %d to %d detected in %s\n", str, lastValue, *pCurrentValue, filename);
	}
