...
typedef STFResult (* PhysicalUnitFactory)(uint32 unitID, uint64 * params, IPhysicalUnit *& physicalUnitInterface);

// Macro to declare the unit creation functions
#define DECLARE_UNIT_CREATION_FUNCTION(funcName) \
	STFResult funcName (VDRUID unitID, uint64 * params, IPhysicalUnit *& physicalUnitInterface);
...
static const uint64 UNITS_DONE		=	0;		


// Marks end of a "Local Unit ID" to "Global Unit ID" mapping
static const uint64 MAPPING_DONE	=	0;

// Marks end of a parameter list
static const uint64 PARAMS_DONE		 =	0;

// Identifies a DWORD parameter
static const uint64 PARAMS_DWORD	 =	1;

// Identifies a STRING parameter
static const uint64 PARAMS_STRING	 =	2;

// Identifies a STRING parameter
static const uint64 PARAMS_POINTER  =	4;
...
#define _DEBUG_VERBOSE_CREATE_UNIT	1
#else
...
#define CREATE_UNIT(id, call)	(uint64)#id, (uint64)#call, (id), (uint64)call
#else
#define CREATE_UNIT(id, call)	(id), (uint64)call
#endif

#if _DEBUG_VERBOSE_CREATE_UNIT
#define CREATE_UNIT_CONDITIONAL(id, call, mask, value) #id, #call, (uint32)((id) & VDRUIDF_CONSTUCTION_CONDITIONAL), (uint64)call, (uint32)mask, (uint32)value
#else
#define CREATE_UNIT_CONDITIONAL(id, call, mask, value) (uint32)((id) & VDRUIDF_CONSTUCTION_CONDITIONAL), (uint64)call, (uint32)mask, (uint32)value
#endif
...
#define STRING_PARAM(str)	PARAMS_STRING,	(uint64)(str)

// array parameter
#define POINTER_PARAM(ptr) PARAMS_POINTER, (PADDR)(ptr)


// Declaration of GlobalBoardConfiguration to
//??? "to" what? The comment breaks off in the middle of the sentence.
extern uint64 GlobalBoardConfig[];
