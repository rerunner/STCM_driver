...
struct TAG : TAGITEM
{
    TAG (uint32 _id, intptr_t _data, int size_ = 0)
    {
        data = _data;
        id = _id;
        skip = 1;
...
///
intptr_t FilterTags(TAG  * tags, uint32 id, uint32 def);
...
#define MKTAG(name, unit, val, type)	\
	inline TAG SET_##name(type x) {return TAG(val | unit | TAG_SET, (intptr_t)(x));}	\
	inline TAG GET_##name(type  &x) {return TAG(val | unit | TAG_GET, (intptr_t)(&x));}	\
	inline TAG QRY_##name(bool  &x) {return TAG(val | unit | TAG_QRY, (intptr_t)(&x));}	\
	inline type VAL_##name(TAG  * tag) {return (type)(tag->data);}	\
	inline type FVAL_##name(TAG  * tags, type def) {return (type)(FilterTags(tags, val | unit | TAG_GET, (intptr_t)def));}	\
...
#define MKITG(name, unit, val, type)	\
	inline TAG SET_##name(uint32 index, type x) {return TAG(val | unit | TAG_SET | (index << 8), (intptr_t)(x));}	\
	inline TAG GET_##name(uint32 index, type  &x) {return TAG(val | unit | TAG_GET | (index << 8), (intptr_t)(&x));}	\
	inline TAG QRY_##name(uint32 index, bool  &x) {return TAG(val | unit | TAG_QRY | (index << 8), (intptr_t)(&x));}	\
	inline type VAL_##name(TAG  * tag) {return (type)(tag->data);}	\
	inline type FVAL_##name(TAG  * tags, type def) {return (type)(FilterTags(tags, val | unit | TAG_GET, (intptr_t)def));}	\
	inline type  & REF_##name(TAG  * tag) {return *(type  *)(tag->data);}	\
	typedef type TTYPE_##name;	\
...
#define MKRTG(name, unit, val, type)	\
	inline TAG SET_##name(type  &x) {return TAG(val | unit | TAG_SET | TAG_REF, (intptr_t)(void *)(&x));}	\
	inline TAG GET_##name(type  &x) {return TAG(val | unit | TAG_GET | TAG_REF, (intptr_t)(void *)(&x));}	\
	inline TAG QRY_##name(bool  &x) {return TAG(val | unit | TAG_QRY | TAG_REF, (intptr_t)(&x));}	\
	inline type VAL_##name(TAG  * tag) {return *(type  *)(tag->data);}	\
	inline type  & REF_##name(TAG  * tag) {return *(type  *)(tag->data);}	\
	typedef type TTYPE_##name;	\
